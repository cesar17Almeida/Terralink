...
actual class BleClient {
    ...
    actual fun scan(saviaOnly: Boolean): Flow<ScannedDevice> = callbackFlow {
        ...
        val saviaParcelUuid = ParcelUuid.fromString(SAVIA_SERVICE_UUID)
        val filters: List<ScanFilter> = if (saviaOnly) {
            listOf(ScanFilter.Builder().setServiceUuid(saviaParcelUuid).build())
        } else {
            emptyList()  // No filter -> every nearby LE advertisement.
        }
            ...
            override fun onScanResult(callbackType: Int, result: ScanResult) {
                ...
                trySend(
                    ScannedDevice(
                        ...
                        rssi = result.rssi,
                        isSavia = saviaParcelUuid in advertisedUuids,
                    )
                )
            }
